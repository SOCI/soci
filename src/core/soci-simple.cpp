...
struct statement_wrapper
{
    ...
    std::map<int, long> into_longlongs;
    ...
    std::map<int, std::vector<long> > into_longlongs_v;
    ...
    std::map<std::string, long> use_longlongs;
    ...
    std::map<std::string, std::vector<long> > use_longlongs_v;
    ...
    statement_wrapper::kind k, char const * type_name)
{
    ...
    if (k == statement_wrapper::single)
    {
        ...
        case dt_unsigned_long_long:
            {
                typedef std::map<std::string, long>::const_iterator
                    iterator;
...
SOCI_DECL long soci_get_into_long_long(statement_handle st, int position)
{
    statement_wrapper * wrapper = static_cast<statement_wrapper *>(st);

    if (position_check_failed(*wrapper,
            statement_wrapper::single, position, dt_long_long, "long") ||
        not_null_check_failed(*wrapper, position))
    {
        return 0L;
    }
...
SOCI_DECL long soci_get_into_long_long_v(statement_handle st, int position, int index)
{
    statement_wrapper * wrapper = static_cast<statement_wrapper *>(st);
    ...
    std::vector<long> const & v = wrapper->into_longlongs_v[position];
    if (index_check_failed(v, *wrapper, index) ||
        not_null_check_failed(*wrapper, position, index))
    {
        return 0;
...
SOCI_DECL void soci_set_use_long_long_v(statement_handle st,
    char const * name, int index, long val)
{
    statement_wrapper * wrapper = static_cast<statement_wrapper *>(st);
    ...
    std::vector<long> & v = wrapper->use_longlongs_v[name];
    if (index_check_failed(v, *wrapper, index))
    {
        return;
    }
...
SOCI_DECL long soci_get_use_long_long(statement_handle st, char const * name)
{
    statement_wrapper * wrapper = static_cast<statement_wrapper *>(st);

    if (name_exists_check_failed(*wrapper,
            name, dt_long_long, statement_wrapper::bulk, "long"))
    {
        return 0L;
    }
...
SOCI_DECL void soci_prepare(statement_handle st, char const * query)
{
    ...
    try
    {
        ...
        }
        {
            ...
            {
                std::string const & use_name = uit->first;
                std::vector<long> & use_longlong = uit->second;
                std::vector<indicator> & use_ind =
                    wrapper->use_indicators_v[use_name];
                wrapper->st.exchange(use(use_longlong, use_ind, use_name));
            }
...
SOCI_DECL long soci_get_affected_rows(statement_handle st)
{
    statement_wrapper * wrapper = static_cast<statement_wrapper *>(st);
