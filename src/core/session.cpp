...
session::session()
    : once(this), prepare(this), query_transformation_(NULL),
      expand_vectors_(true), log_params_(false), log_endl_(false), logStream_(NULL), 
...
void session::set_log_endl(bool enable)
{
    if (isFromPool_)
    {
        pool_->at(poolPosition_).set_log_endl(enable);
    }
    else
    {
        log_endl_ = enable;
...
bool session::log_endl() const
{
    if (isFromPool_)
    {
        return pool_->at(poolPosition_).log_endl();
    }
    else
    {
        return log_endl_;
...
void session::set_log_params(bool enable)
{
    if (isFromPool_)
    {
        pool_->at(poolPosition_).set_log_params(enable);
    }
    else
    {
        log_params_ = enable;
...
void session::expand_vector_params(bool enable)
{
    if (isFromPool_)
    {
        pool_->at(poolPosition_).set_log_params(enable);
    ...
    {
        expand_vectors_ = enable;
    }
}

bool session::expand_vector_params() const
{
    if (isFromPool_)
    {
        return pool_->at(poolPosition_).log_params();
