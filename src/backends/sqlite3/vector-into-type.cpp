...
void set_number_in_vector(void *p, int idx, const sqlite3_column &col)
{
    ...
    switch (col.type_)
    {
        ...
        case dt_double:
            set_in_vector(p, idx, static_cast<T>(col.floating_));
            break;
...
void sqlite3_vector_into_type_backend::post_fetch(bool gotData, indicator * ind)
{
    ...
    {
        const sqlite3_column &col = statement_.dataCache_[i][position_-1];
        ...
        switch (type_)
        {
            case x_char:
            {
                switch (col.type_)
                {
                    ...
                    case dt_double:
                        set_in_vector(data_, i, double_to_cstring(col.floating_)[0]);
                        break;
            ...
            case x_stdstring:
            {
                switch (col.type_)
                {
                    ...
                    case dt_double:
                        set_in_vector(data_, i, double_to_cstring(col.floating_));
                        break;
