...
public:
    ...
    virtual bool get_next_sequence_values(std::string const&, std::vector<long long>&)
...
* `get_next_sequence_value`, `get_next_sequence_values`, `get_last_insert_id` - Called to retrieve either one or many values from a sequence or auto-generated ID value and every backend should define at least one of them to allow the code using auto-generated values to work.
