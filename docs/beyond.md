...
SOCI tries to help to deal with this unfortunate situation by providing two functions: `session::get_next_sequence_value()` (and its variant `get_next_sequence_values()` which should be used instead of calling this function many times in a loop) and `session::get_last_insert_id`.
